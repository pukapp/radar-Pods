...
- (instancetype)init_throws_withData:(NSData *)serialized
{
    if (self = [super init]) {
        ...
        Byte version; //3
        NSLog(@"serialized.length: %ld", serialized.length); // 299
        [serialized getBytes:&version length:VERSION_LENGTH];

        NSUInteger messageAndMacLength; //210
        ...
        OWSLogDebug(@"whisperMessage.ciphertext.length %ld", whisperMessage.ciphertext.length);
