...
- (NSData *)removePadding {
    OWSLogDebug(@"removePadding before: %ld", self.length);
    ...
    NSData *res = [self subdataWithRange:NSMakeRange(0, paddingStart)];
    OWSLogDebug(@"removePadding after: %ld", res.length);
    return res;
}
